...
def read_chat(chat): 
   #llm = Llama(model_path="./convgpt4all.bin", n_ctx=max_total_ctx)
    prompt = f'''Goal: "Decide whether or not JFK-bot could respond."
    Context: "JFK-bot is an AI chatbot version of former president John F. Kennedy. JFK-bot is in a Discord server, and is fairly chatty, tending to respond whenever possible."
    Recent Chat History: "{chat}"
    Response Guidelines: "JFK-bot is chatty, witty, verbose, humorous, and even understands modern humor & dark humor. JFK-bot makes a response whenever there is an opportunity to do so."
    ...
    output = llm(prompt, echo=False, max_tokens=token_gen)
...
def generate_response(chat): 
    ...
    prompt = f'''Goal: Generate a chat response message for JFK-bot to send.
    ...
    output = llm(prompt, echo=False, max_tokens=return_tokens)
    response = output.get('choices')[0].get('text')
